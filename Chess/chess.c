...
U64 bitboards[12]; // here.
U64 occboards[3]; // here.
int side;
...
static inline int is_square_attacked(int square, int side) {
    ...
    if (get_bishop_attacks(square, occboards[both]) & ((side == white) ? bitboards[B] : bitboards[b])) {return 1;}
    if (get_rook_attacks(square, occboards[both]) & ((side == white) ? bitboards[R] : bitboards[r])) {return 1;} 
    if (get_queen_attacks(square, occboards[both]) & ((side == white) ? bitboards[Q] : bitboards[q])) {return 1;}
...
static inline int makemove(int move, int move_flag) {    
    ...
    if (enpass)
    {
        (side == white) ? popbit(bitboards[p], target_square + 8) :
                            popbit(bitboards[P], target_square - 8);
    }
